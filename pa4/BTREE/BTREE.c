...
void _traverse(BTreeNode *present) {
  // There are n keys and n+1 children, travers through n keys and first n
  // children
  int i;
...
BTreeNode *_splitChild(BTreeNode *present) {
  ...
  if (present->P != NULL) {
    ...
    for (parentIndex = 0; parentIndex < currentParent->n + 1 &&
                          currentParent->C[parentIndex] != present;
         parentIndex++)
      ;
...
void _balancingAfterDel(BTreeNode *present) // repairAfterDelete
{
  ...
  if (present->n < minKeys) {
    if (present->P == NULL) {
      ...
      } else if (parentIndex < parent->n &&
                 parent->C[parentIndex + 1]->n > minKeys) {
...
void _mappingNodes(BTreeNode *present, BTreeNode ***nodePtr, int *numNodes,
                   int level) {
  int i;
  if (present == NULL)
    return;
