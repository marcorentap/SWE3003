...
BTreeNode* search(int k) { return (root == NULL) ? NULL : _search(root, k); }
...
BTreeNode* _createNode(bool _leaf) {
    ...
    // Initialize child
    for (i = 0; i < t + 2; i++) newNode->C[i] = NULL;
...
void _traverse(BTreeNode* present) {
    ...
    for (i = 0; i < present->n; i++) {
        ...
        if (present->leaf == false) _traverse(present->C[i]);
...
BTreeNode* _search(BTreeNode* present, int k) {
    ...
    if (present->keys[i] == k) return present;
...
BTreeNode* _splitChild(BTreeNode* present) {
    ...
    if (present->P != NULL) {
        ...
        for (parentIndex = 0; parentIndex < currentParent->n + 1 && currentParent->C[parentIndex] != present;
             parentIndex++);
    ...
    if (search(k) == NULL) {
        ...
        if (present->keys[a] == k) {
            if (present->leaf) {
                ...
                BTreeNode* sib = present->C[a];

                while (!sib->leaf) {
                    sib = sib->C[sib->n];
                }

                present->keys[a] = sib->keys[sib->n - 1];
                sib->n--;
...
void _getNumberOfNodes(BTreeNode* present, int* numNodes, int level) {
    ...
    if (present->leaf == false) {
        for (i = 0; i < present->n + 1; i++) _getNumberOfNodes(present->C[i], numNodes, level + 1);
    }
    numNodes[level] += 1;
}
...
void _mappingNodes(BTreeNode* present, BTreeNode*** nodePtr, int* numNodes, int level) {
    ...
    if (present->leaf == false) {
        for (i = 0; i < present->n + 1; i++) _mappingNodes(present->C[i], nodePtr, numNodes, level + 1);
    }
